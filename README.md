...
vLLM runs Gemma 4's multi-token prediction while `MTP_SPECULATIVE_TOKENS` in `card/manifest.env` is above 0, and it
is 3 since the owner's choice of 2026-09-26: `--speculative-config` with method `mtp`, where Google's assistant for
the 31B, pinned in the manifest as the weights are, drafts that many tokens a step, and the heretic verifies every
drafted token in one pass and keeps each only as its own sampling would, so the distribution of the answers does not
change, only their speed. The drafter reads the heretic's KV cache and keeps none of its own; its 0.9 GB of weights
come out of the cache's share of the memory. `/v1/state` names the drafter's revision and the count among the
versions, and so does the smoke's record.

A card prepared with another count is not prepared for this manifest, and the launcher starts nothing there until the
preparation runs again. Copy the checkout to the card again, and run the preparation with nothing on stdin, since the
card holds the keys. It fetches the drafter into `drafters/<revision>` of the card's state and checks every hash
again, while the pair it started before serves on. Then end that pair and start the card, which loads the heretic
with the drafter:
